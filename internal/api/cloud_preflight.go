...
func cloudDeletionPreflightHandler(reader cloudDeletionPreflightReader) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil || version < 1 || strconv.FormatInt(version, 10) != versionText || !handoffUUID(in.OrganizationID) || !handoffUUID(in.OwnerUserID) ||
			len(c.Request.Header.Values("X-Billing-Ownership-Version")) != 1 || len(c.Request.Header.Values("X-Billing-Owner-User-ID")) != 1 {
